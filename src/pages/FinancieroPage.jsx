...
export default function FinancieroPage() {
  ...
  const contratosVencidosIds    = contratos.filter(c => c.estatus === 'vencido').map(c => c.id);
  const contratosCanceladosIds  = contratos.filter(c => c.estatus === 'cancelado').map(c => c.id);
  const contratosFinalizadosIds = contratos.filter(c => c.estatus === 'finalizado').map(c => c.id);

  const pagosFinalizados = pagosFiltrados.filter(p => contratosFinalizadosIds.includes(p.contrato_id));
  const pagosActivos     = pagosFiltrados.filter(p => contratosActivosIds.includes(p.contrato_id));
  const pagosVencidos    = pagosFiltrados.filter(p => contratosVencidosIds.includes(p.contrato_id));
  const pagosCancelados  = pagosFiltrados.filter(p => contratosCanceladosIds.includes(p.contrato_id));
  ...
  // ── Finalizar contrato ───────────────────────────────────
  const finalizarContrato = async (contrato_id) => {
    const pagosDelContrato = pagos.filter(p => p.contrato_id === contrato_id);
    const tienePendientes  = pagosDelContrato.some(
      p => p.estado === 'pendiente' || p.estado === 'parcial'
    );
  ...
  const TablaPagosDesktop = ({ lista, conAccion = false, onFinalizar = null }) => {
    ...
    const puedeFinalizarContrato = (p) => {
      const pagosDelContrato = lista.filter(x => x.contrato_id === p.contrato_id);
      const todosCompletos   = pagosDelContrato.every(x => x.estado === 'pagado');
      const esUltimo         = [...lista].reverse().find(x => x.contrato_id === p.contrato_id)?.id === p.id;
      return todosCompletos && esUltimo;
    };
                  ...
                  {conAccion && (
                    <td>
                      <div style={{ display: 'flex', gap: 6 }}>
                        ...
                        {onFinalizar && puedeFinalizarContrato(p) && (
                          <button
                            className="btn-expediente"
                            onClick={() => onFinalizar(p.contrato_id)}
                            title="Marcar contrato como finalizado"
    ...
    return (
      <div className="fin-mobile">
        {lista.map(p => {
          const estilo   = ESTADO_COLORS[p.estado] || { color: '#888', bg: '#f3f4f6' };
          const diff     = Number(p.diferencia || 0);
          const puedeFin = puedeFinalizarContrato(p);
          return (
            <div className="pago-card" key={p.id}>
              <div className="pago-card-header">
                <span className="pago-card-periodo">{p.periodo}</span>
            ...
            <div className="table-card financiero-group">
              ...
              {vencidosExpandido && (
                pagosVencidos.length === 0 ? (
                  ...
                  <>
                    <TablaPagosDesktop lista={pagosVencidos} conAccion onFinalizar={finalizarContrato} />
                    <TablaPagosMobile  lista={pagosVencidos} conAccion onFinalizar={finalizarContrato} />
                  </>
                )
              )}
            </div>
          ...
          <div style={{ marginTop: '1rem' }}>
            <div className="table-card financiero-group">
              <button
                className="financiero-expand-btn"
                onClick={() =>
                  setExpandido(prev => ({ ...prev, cancelados: !prev.cancelados }))
                }
              >
                <span className="financiero-icon">
                  {expandido.cancelados ? '▼' : '▶'}
